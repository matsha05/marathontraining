        ...
        <div className={`v3-card p-6 ${className}`}>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="text-center">
                    ...
                    <p className="text-[10px]" style={{ color: 'var(--text-subtle)' }}>Avg Feel</p>
                </div>
                <div className="text-center">
                    <p className="text-2xl font-mono" style={{ color: 'var(--color-accent)' }}>{insights.longestStreak}</p>
                    <p className="text-[10px]" style={{ color: 'var(--text-subtle)' }}>Best Streak</p>
                </div>
            </div>

            {/* Effort Calibration (Seiler) */}
            <div className="flex items-center gap-4 p-3 rounded-lg mb-4" style={{ background: 'var(--bg-elevated)' }}>
                <div className="flex-1">
                    <p className="text-[10px]" style={{ color: 'var(--text-subtle)' }}>Easy days feel</p>
                ...
                <div className="flex-1">
                    <p className="text-[10px]" style={{ color: 'var(--text-subtle)' }}>Hard days feel</p>
                    <p className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>
                ...
                <div className="space-y-2">
                    {insights.redFlags.map((flag, i) => (
                        ...
                        >
                            ...
                            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{flag.message}</p>
                        </div>
                    ))}
                </div>
            )}
            ...
            {!hasRedFlags && (
                <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-muted)' }}>
                    <svg className="w-4 h-4" style={{ color: insights.feelTrend === 'improving' ? 'var(--color-accent)' : insights.feelTrend === 'declining' ? '#f59e0b' : 'var(--text-muted)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
